...
```{r}
admissions_tble %>% 
  #.keep_all = T means we want to keep all variables
...
```{r}
patients_tble %>% 
  # there is a spike in value where anchor age = 0, obscures our graph
  filter(anchor_age != 0) %>% 
  select(anchor_age) %>% 
  ggplot(aes(x = anchor_age)) +
  geom_histogram(mapping = aes(y = ..density..), 
                 position = "identity", fill = "orange") +
  geom_density(alpha = 0.6) + #shows the smoothing pattern
  theme_classic()
```
