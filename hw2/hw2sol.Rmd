...
for (i in itemids){
  print(dlabitmes_tble$label[dlabitmes_tble$itemid == i])
  labels[j, 1] <- i
...
```{r}
labevents2 <- as_tibble(fread(str_c(mimic_path2, "/labevents_filtered_itemid.csv.gz")))

names(labevents2) <- c("subject_id", "hadm_id", "itemid", "charttime", "valuenum")
...
**Solution**:
chartevents.csv.gz is such a big file, I will read in the smaller file that Dr. Zhou created which has only the events of interest:

```{r}
chartevents1 <- as_tibble(fread(str_c(mimic_path2, "/chartevents_filtered_itemid.csv.gz")))
names(chartevents1) <- c("subject_id", "hadm_id", "stay_id", "charttime", "itemid", "valuenum")
print(chartevents1, width = Inf)
```

first, we only want some vital information, so start with dictionary
...
for (i in vitalids) {
  print(dchart_tble$label[dchart_tble$itemid == i])
...
First I'm going to make a mini tibble for admissions, icustays, patients, labevents, and chartevents.
```{r}
admit <- head(admissions_tble, n = 100)
icu <- icustays_tble %>% 
  arrange(subject_id) %>% 
  head(n = 50)
patients <- head(patients_tble)
labevents <- head(labevents_tble)
chartevents <- head(chartevents1)
```

First, we just want to have the first icustay of each unique patient
```{r}
icu_unique <- icu %>% 
  arrange(subject_id) %>% # this just makes it easier for me (not necessary)
  group_by(subject_id) %>% 
  slice_min(intime) %>% 
...
So above, we have made an (abbreviated) data set that only has the first icu stay of each unique patient along with their admission and patient info + only adults.

So now to filter the labevents table so that we can join that into the above data set: (only keep first lab measurement for the above icu stays (match subject id and hospital stay id))

```{r}
labmatch <- labevents_tble %>% 
  # only keeping rows matching the subject and hospital stay id from above data set
  semi_join(icu_unique, by = c("subject_id", "hadm_id")) %>% 
  # only want lab measurement taken after patient was admitted to the hospital:
  left_join(select(icu_unique, subject_id, hadm_id, intime), by = c("subject_id", "hadm_id")) %>% 
  mutate(charttime = ymd_hms(charttime), intime = ymd_hms(intime)) %>% 
...
for (i in 1:11) {
  for (j in 1:11) {
    if (names(labmatch)[2+i] == labels[j, 1]) {
      names(labmatch)[2+i] <- labels[j, 2]
    }
  }
}
```
...
```{r}
chartmatch <- chartevents1 %>% 
  semi_join(icu_unique, by = c("subject_id", "hadm_id")) %>% 
  left_join(select(icu_unique, subject_id, hadm_id, intime), by = c("subject_id", "hadm_id")) %>% 
  mutate(charttime = ymd_hms(charttime), intime = ymd_hms(intime)) %>% 
  filter(charttime >= intime) %>% 
  arrange(subject_id, charttime) %>%  # not necessary, just for me
  group_by(subject_id, itemid) %>% 
  arrange(charttime, .by_group = TRUE) %>% 
  slice_head() %>% 
  select(-charttime, -intime, -stay_id) %>% 
  spread(key = itemid, value = valuenum) %>% 
  #arrange(subject_id) %>% # not necessary, just for me
  print()
```

```{r}
print(chartmatch, width = Inf)
```
...
for (i in 1:7) {
  for (j in 1:7) {
    if (names(chartmatch)[2+i] == vitallabs[j, 1]) {
      names(chartmatch)[2+i] <- vitallabs[j, 2]
    }
  }
}
```
...
Now to combine all of these things:

```{r}
combined <- icu_unique %>% 
  full_join(labmatch, by = c("subject_id", "hadm_id")) %>% 
  full_join(chartmatch, by = c("subject_id", "hadm_id")) %>% 
  print()
```
