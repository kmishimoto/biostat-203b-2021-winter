...
```{r}
icu_pat_adm <- icustays_tble %>% 
  group_by(subject_id) %>% 
  slice_min(intime) %>% # taking only the first ICU stay of each unique patient
...
```{r}
lab_sub <- labevents_tble %>% 
  semi_join(icu_uniq, by = c("subject_id", "hadm_id")) %>% 
  collect()
```

```{r}
chart_sub <- chartevents_tble %>% 
  semi_join(icu_uniq, by = c("subject_id", "hadm_id", "stay_id")) %>% 
  collect()
```
...
```{r}
cohort <- icu_pat_adm %>% 
  left_join(lab_sub, by = c("subject_id", "hadm_id")) %>% 
  left_join(chart_sub, by = c("subject_id", "hadm_id", "stay_id")) %>% 
...
icu_cohort <- icustays_tble %>% 
  ...
  mutate(death30days = (deathtime - admittime) < ddays(30)) %>%
  print(width = Inf)
  # saveRDS(file = "mimciv_shiny/icu_cohort.rds")
```
