...
I followed the outline of steps provided by Dr. Zhou:
...
icu_cohort <- icustays_tble %>% 
  # we only want the first stay for each patient:
  group_by(subject_id) %>% 
  filter(rank(intime)== 1) %>% 
  ungroup(subject_id) %>% # we don't want this to be a grouped tibble
  # merging in the admissions and patients tables:
  left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>% 
  left_join(patients_tble, by = "subject_id") %>% 
  # we only want adults:
  filter((year(admittime) - anchor_year + anchor_age) >= 18) %>% 
  #merging in labevents and chartevents tables:
  ...
  show_query() %>%
  # we want the output to be a tibble:
  as_tibble() %>% 
  # we want to force a full query (rather than lazy evaluation):
  collect() %>% 
  # creating two new variables (30 day death indicator and age at admission)
  mutate(death30days = (deathtime - admittime) < ddays(30), 
    admit_age = year(admittime) - anchor_year + anchor_age) %>% 
  # I want to see the resulting tibble:
  print(width = Inf) %>% 
  # saving tibble as a .rds file so we can use it in the shiny app:
...
The shiny app is in the `mimiciv_shiny` folder.
